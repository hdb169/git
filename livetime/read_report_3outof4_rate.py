...
rf = glob.glob('/lustre/expphy/volatile/hallc/spring17/hdbhatt/group/REPORT_OUTPUT/COIN/PRODUCTION/replay_coin_*-1.report') 
...
for index, run in enumerate(rf):
    with open(rf[index]) as fobj:
        for line in fobj:
            ...
            if('HMS 3/4 Trigger Rate' in report_data[0]) : rfd['hms_rate'].append(report_data[1][:7].strip())  # [1][:7] is for 
