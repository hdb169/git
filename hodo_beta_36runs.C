...
void hodo_beta_36runs(){
  ...
  for(Int_t i=0; i<NRUNS; i++){
    ...
    t[i]->Draw(Form("H.hod.beta>>h1[%d]", i ),"H.gtr.dp>-10 &&H.gtr.dp <12 ");

    c1[i]->cd(2);gPad->SetGridx();
    t[i]->Draw(Form("H.gtr.dp:H.hod.beta>>h2[%d]", i ), "H.gtr.dp>-10 &&H.gtr.dp <12 ","colz");

    c1[i]->cd(3);gPad->SetGridx();
    t[i]->Draw(Form("P.hod.beta>>h3[%d]", i ),"P.gtr.dp>-10 &&P.gtr.dp <20 && P.aero.npeSum >1 &&P.cal.eprtracknorm < 0.2 ");

    c1[i]->cd(4);gPad->SetGridx();
    t[i]->Draw(Form("P.gtr.dp:P.hod.beta>>h4[%d]", i ), "P.gtr.dp>-10 &&P.gtr.dp <20 &&  P.aero.npeSum >1 &&P.cal.eprtracknorm < 0.2","colz");
