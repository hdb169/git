1)This is just a trial, may not be correct...because other detectors can not exactly selcet electrons or pions, so need to apply pi to e ratio or similar to get
efficiency....
2)This script can not be used for all runs as RF offset is different for different kinematics.	
...
void cal_eff(){
  ...
  Int_t runs[NRUNS]= { 6429-6433, 6459 -6464};
  gROOT->SetBatch("TRUE");
