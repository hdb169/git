...
This works for shms singles 5347-50, remove ps factor
#include <TSystem.h>
#include <TString.h>
#include "TFile.h"
#include "TTree.h"
...
//void make_hist_shms_lumi(TString basename="",Int_t nrun=2043){
  ...
  ofstream txtfile1("txtfile_shms/shms_livetime_comp.txt",ios::app);
  ofstream txtfile2("txtfile_shms/shms_livetime_edtm.txt",ios::app);
  ofstream txtfile3("txtfile_shms/shms_tracking_eff_nopid.txt",ios::app);
  ofstream txtfile4("txtfile_shms/shms_tracking_eff_pid.txt",ios::app);
  ofstream txtfile5("txtfile_shms/shms_yield_notrack.txt",ios::app);
  
  ofstream txtfile6("txtfile_shms/shms_yield_track_clt_uncorr.txt",ios::app);
  ofstream txtfile7("txtfile_shms/shms_yield_track_clt_corr.txt",ios::app);
  ofstream txtfile8("txtfile_shms/shms_yield_track_edtmlt_uncorr.txt",ios::app);
  ofstream txtfile9("txtfile_shms/shms_yield_track_edtmlt_corr.txt",ios::app);
  
  ofstream txtfile10("txtfile_shms/shms_yield_elclean_clt.txt",ios::app);
  ofstream txtfile11("txtfile_shms/shms_yield_elreal_clt.txt",ios::app);
  ofstream txtfile12("txtfile_shms/shms_current_cut_uncorr.txt",ios::app);
  ofstream txtfile13("txtfile_shms/shms_current_cut_corr.txt",ios::app);


void shms_yield_final(){
...
TH1F* h_typ2_evts = new TH1F(" h_typ2_evts"," ;  h_typ2_evts ",100, -2, 8.);
    ...
    Double_t comp_lt = good_evts/(tot_scal_cut_TRIG2) ;
    ...
    Double_t yield_lt_notrack = good_counts_notrack*ps[r]/(charge_sum_cut_corr* comp_lt);//no delta and no ntrack>0 cut
    Double_t yield_lt_notrack_err =yield_lt_notrack * sqrt(1./good_counts_notrack);

    ///////////////uncorr charge
    Double_t yield_clt_track_uncorr = good_counts_track*ps[r]/(charge_sum_cut* comp_lt * tr_eff_nopid);//delta and ntrack>0 
    Double_t yield_clt_track_uncorr_err = yield_clt_track_uncorr * sqrt(1./good_counts_track);

    Double_t yield_edtmlt_track_uncorr = good_counts_track*ps[r]/(charge_sum_cut* edtm_lt * tr_eff_nopid);//delta and ntrack>0 
    Double_t yield_edtmlt_track_uncorr_err = yield_edtmlt_track_uncorr * sqrt(1./good_counts_track);


    //corrected charge
    Double_t yield_clt_track_corr = good_counts_track*ps[r]/(charge_sum_cut_corr* comp_lt * tr_eff_nopid);//delta and ntrack>0 
    Double_t yield_clt_track_corr_err = yield_clt_track_corr * sqrt(1./good_counts_track);

    Double_t yield_edtmlt_track_corr = good_counts_track*ps[r]/(charge_sum_cut_corr* edtm_lt * tr_eff_nopid);//delta and ntrack>0 
    Double_t yield_edtmlt_track_corr_err = yield_edtmlt_track_corr * sqrt(1./good_counts_track);

    ////////////////done
   

    Double_t Elclean_yield = tot_scal_cut_hEL_CLEAN*ps[r]/charge_sum_cut_corr; //bare yield
    Double_t Elclean_yield_err =  Elclean_yield *sqrt(1./tot_scal_cut_hEL_CLEAN);


    Double_t Elreal_yield = tot_scal_cut_hEL_REAL*ps[r]/charge_sum_cut_corr;//bare yield
    Double_t Elreal_yield_err = Elreal_yield * sqrt(1./ tot_scal_cut_hEL_REAL);

    Double_t Elreal_rate = tot_scal_cut_hEL_REAL/tot_scal_cut_time;//time cut
    
    cout<< "Real_scal_count = " << tot_scal_cut_hEL_REAL<<endl;
    cout<<" Tot cut time = "<< tot_scal_cut_time<<endl;
    cout<<" Rate = "<< Elreal_rate/1000.<<endl;
    ...
    txtfile1<< runs[r]<<"    "<< cut_current_corr<< "    "<<Elreal_rate<<        "    "<< comp_lt<<                 "    "<< comp_lt_err<<endl;
    txtfile2<< runs[r]<<"    "<< cut_current_corr<< "    "<<Elreal_rate<<        "    "<< edtm_lt <<                "    "<< edtm_lt_err<< endl;
    txtfile3<< runs[r]<<"    "<< cut_current_corr<< "    "<<Elreal_rate<<        "    "<< tr_eff_nopid<<            "    "<< tr_eff_nopid_err <<endl;
    txtfile4<< runs[r]<<"    "<< cut_current_corr<< "    "<<Elreal_rate<<        "    "<< tr_eff_pid <<             "    "<< tr_eff_pid_err<<endl;
    txtfile5<< runs[r]<<"    "<< cut_current_corr<< "    "<<Elreal_rate<<        "    "<< yield_lt_notrack <<       "    "<< yield_lt_notrack_err<<endl;

    txtfile6<< runs[r]<<"    "<< cut_current_corr<<"    "<<Elreal_rate<<        "    "<< yield_clt_track_uncorr <<  "    "<< yield_clt_track_uncorr_err<<endl;
    txtfile7<< runs[r]<<"    "<< cut_current_corr<<"    "<<Elreal_rate<<        "    "<< yield_clt_track_corr <<    "    "<< yield_clt_track_corr_err<<endl;
    txtfile8<< runs[r]<<"    "<< cut_current_corr<<"    "<<Elreal_rate<<        "    "<< yield_edtmlt_track_uncorr<<"    "<< yield_edtmlt_track_uncorr_err<<endl;
    txtfile9<< runs[r]<<"    "<< cut_current_corr<<"    "<<Elreal_rate<<        "    "<< yield_edtmlt_track_corr << "    "<< yield_edtmlt_track_corr_err<<endl;

    
    
    txtfile10<< runs[r]<<"    "<< cut_current_corr<<"    "<<Elreal_rate<<        "    "<< Elclean_yield <<           "    "<< Elclean_yield_err <<endl;
    txtfile11<< runs[r]<<"    "<< cut_current_corr<<"    "<<Elreal_rate<<        "    "<< Elreal_yield <<            "    "<< Elreal_yield_err<<endl;
    txtfile12<< runs[r]<<"    "<<cut_current_uncorr<<"   "<<charge_sum_cut<<endl;
    txtfile13<< runs[r]<<"    "<<cut_current_corr<< "    "<<charge_sum_cut_corr<<endl;
