...
void shms_6482_85(){
  ...
  */
    ...
    Double_t yield_lt_notrack = good_counts_notrack*ps[r]/(charge_sum_cut* comp_lt);//no delta and no ntrack>0 cut
    Double_t yield_lt_notrack_err =yield_lt_notrack * sqrt(1./good_counts_notrack);

    ///////////////uncorr charge
    Double_t yield_clt_track  = good_counts_track*ps[r]/(charge_sum_cut* comp_lt * tr_eff_pid);//delta and ntrack>0 
    Double_t yield_clt_track_err = yield_clt_track  * sqrt(1./good_counts_track);

    Double_t yield_edtmlt_track  = good_counts_track*ps[r]/(charge_sum_cut* edtm_lt * tr_eff_pid);//delta and ntrack>0 
    Double_t yield_edtmlt_track_err = yield_edtmlt_track  * sqrt(1./good_counts_track);
   
    ////////////////done
    // cout<<"ps = "<<ps[r]<<endl;

    Double_t Elclean_yield = tot_scal_cut_hEL_CLEAN/charge_sum_cut; 
    Double_t Elclean_yield_err =  Elclean_yield *sqrt(1./tot_scal_cut_hEL_CLEAN);


    Double_t Elreal_yield = tot_scal_cut_hEL_REAL/charge_sum_cut;
    ...
    Double_t Elreal_yield_elt = tot_scal_cut_hEL_REAL/(charge_sum_cut * factor_ptig1_ele_lt) ;//bare yield
    Double_t Elreal_yield_elt_err = Elreal_yield_elt * sqrt(1./ tot_scal_cut_hEL_REAL);

   
    Double_t Elclean_yield_elt = tot_scal_cut_hEL_CLEAN/(charge_sum_cut * factor_ptig1_ele_lt) ;//bare yield
    Double_t Elclean_yield_elt_err = Elclean_yield_elt * sqrt(1./ tot_scal_cut_hEL_CLEAN);

    Double_t yield_clt_track_elt  = good_counts_track*ps[r]/(charge_sum_cut* comp_lt * tr_eff_pid * factor_ptig1_ele_lt );//delta and ntrack>0 
    ...
    */
    ...
    /*
      ...
      txtfile5<< runs[r]<<"    "<< cut_current<<"    "<<PTRIG1_Rate<<        "    "<< yield_edtmlt_track << "    "<< yield_edtmlt_track_err<<endl;
      txtfile6<< runs[r]<<"    "<<cut_current<< "    "<<PTRIG1_Rate<<        "    "<<charge_sum_cut<<endl;
      txtfile7<< runs[r]<<"    "<< cut_current<<"    "<<PTRIG1_Rate<<        "    "<<  Elreal_yield   <<"    "<<   Elreal_yield_err  <<endl;
      txtfile8<< runs[r]<<"    "<< cut_current<<"    "<<PTRIG1_Rate<<        "    "<<  Elclean_yield   <<"    "<<   Elclean_yield_err  <<endl;
    */
    ...
    // txtfile4<< runs[r]<<"    "<< cut_current<<"    "<<PTRIG1_Rate<<        "    "<< yield_clt_track_elt <<    "    "<< yield_clt_track_elt_err<<endl;
    // txtfile7<< runs[r]<<"    "<< cut_current<<"    "<<PTRIG1_Rate<<        "    "<<  Elreal_yield_elt   <<"    "<<   Elreal_yield_elt_err  <<endl;
    // txtfile8<< runs[r]<<"    "<< cut_current<<"    "<<PTRIG1_Rate<<        "    "<<  Elclean_yield_elt   <<"    "<<   Elclean_yield_elt_err  <<endl;
