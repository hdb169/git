...
void read_yield_elreal_vs_rate()//------------------------------1
{
#include "gStyle.h"
  gStyle->SetStatX(0.9);
  gStyle->SetStatY(0.9);
  ...
  std::ifstream infile3("../txtfile_shms/shms_yield_elreal.txt");//------------------------2
  ...
  gr1->SetLineWidth(3);
  gr1->GetXaxis()->SetTitle("Elreal Rate (kHz)");//----------------------------------------------1
  gr1->GetYaxis()->SetTitle("Yield Elreal (counts/uC)");//---------------2
  ...
  TString title = "SHMS (e-) : D2: 5347-5350:  Yield = Elreal Counts/(Q)  :";//-------------------------3
  ...
  cout<<slop<<endl;
  c3->Print("pdf/yield_shms_5347_5350_elreal_vs_rate.pdf");//---------------------------------------7
