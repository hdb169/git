...
void read_yield_edtmlt_tr_vs_cur()//------------------------------6
{
#include "gStyle.h"
  gStyle->SetStatX(0.9);
  gStyle->SetStatY(0.9);
  ...
  std::ifstream infile3("../txtfile_shms/shms_yield_track_edtmlt.txt");//------------------------5
  ...
  gr1->GetYaxis()->SetTitle("Yield (Tracked)  Edtmlt Tr (counts/uC)");//---------------2
  ...
  gStyle->SetOptFit();
  TString title = "SHMS (e-) : D2: 5347-5350:  Yield = Good Counts/(Q *edtmLT* TrEff.)  :";//-------------------------3
  ...
  cout<<slop<<endl;
  c3->Print("pdf/yield_shms_5347_5350_edtmlt_tr_vs_curr.pdf");//---------------------------------------7
