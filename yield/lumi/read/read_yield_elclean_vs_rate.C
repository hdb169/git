...
void read_yield_elclean_vs_rate()//------------------------------1
{
#include "gStyle.h"
  gStyle->SetStatX(0.9);
  gStyle->SetStatY(0.9);
  ...
  std::ifstream infile3("../txtfile_shms/shms_yield_elclean.txt");//------------------------2
  ...
  cout<< "I am good"<<endl;
  auto gr1 = new TGraphErrors(run3.size(), &rate[0], &yield[0], 0, &error[0]);//-----------------3
  ...
  gr1->SetLineWidth(3);
  gr1->GetXaxis()->SetTitle("Elclean Rate (kHz)");//----------------------------------------------1
  gr1->GetYaxis()->SetTitle("Yield Elclean (counts/uC)");//---------------2
  ...
  TString title = "SHMS (e-) : D2: 5347-5350:  Yield = Elclean Counts/(Q)  :";//-------------------------3
  ...
  cout<<slop<<endl;
  c3->Print("pdf/yield_shms_5347_5350_elclean_vs_rate.pdf");//---------------------------------------7
