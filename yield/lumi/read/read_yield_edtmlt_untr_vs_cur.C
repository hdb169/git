...
void read_yield_edtmlt_untr_vs_cur()//------------------------------6
{
#include "gStyle.h"
  gStyle->SetStatX(0.9);
  gStyle->SetStatY(0.9);
  ...
  std::ifstream infile3("../txtfile_shms/shms_yield_notrack_edtmlt.txt");//------------------------5
  ...
  cout<< "I am good"<<endl;
  auto gr1 = new TGraphErrors(run3.size(), &current[0], &yield[0], 0, &error[0]);//-----------------4
  ...
  gr1->GetYaxis()->SetTitle("Yield (Untracked)  Edtmlt  (counts/uC)");//---------------2
  ...
  gStyle->SetOptFit();
  TString title = "SHMS (e-) : D2: 5347-5350:  Yield = Good Counts no track/(Q *edtmLT)  :";//-------------------------3
  ...
  cout<<slop<<endl;
  c3->Print("pdf/yield_shms_5347_5350_edtmlt_untr_vs_curr.pdf");//---------------------------------------7
