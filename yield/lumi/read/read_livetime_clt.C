...
#include "gStyle.h"
  ...
  std::ifstream infile3("../txtfile_shms/shms_livetime_comp.txt");//------------------------2
  ...
  vector <double> rate;
  vector <double> complt;
  vector <double> complt_err;
  ...
  cout<< "I am good"<<endl;
  auto gr1 = new TGraphErrors(run3.size(), &current[0], &complt[0], 0, &complt_err[0]);//-----------------3
  ...
  gr1->SetLineWidth(3);
  gr1->GetXaxis()->SetTitle("Current (uA)");//----------------------------------------------1
  gr1->GetYaxis()->SetTitle("COMP LT");//---------------2
  gr1->GetXaxis()->CenterTitle();
  gr1->GetYaxis()->CenterTitle();
  gr1->Draw("AP");
  gr1->Fit("pol1","Q","",30, 70);

  gr1->SetMarkerStyle(20);
  gr1->SetMarkerSize(2.5);
  gStyle->SetOptFit();

  TString title = "SHMS (e-) : D2: 5347-5350:  COMPLT = Typelo1_evts/scal_TRIG2_events :";//-------------------------3
  ...
  cout<<slop<<endl;
  c3->Print("pdf/yield_shms_5347_5350_edtmlt.pdf");//---------------------------------------7
