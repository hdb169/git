...
//void SIDIS_with_dummy(Int_t runnumber, Int_t targ=1, Int_t pisign=1, Int_t kingrp=1, Int_t ptgrp=1){
void sidis_kine_compare(Int_t numruns, Int_t runnumber,  Int_t targ, Int_t pisign, Int_t kingrp, Int_t ptgrp){
  // TFile *ff = new TFile("/w/hallc-scifs17exp/c-sidis18/SIDISAnalyzer/scripts/ROOT/SIDIS_with_dummy_LD2_2_3.root", "RECREATE");      

  TString ROOT_FILE_PATH ="/lustre/expphy/volatile/hallc/spring17/hdbhatt/ROOTfiles/";
  TString SIMC_ROOT_FILE_PATH = "/lustre/expphy/volatile/hallc/spring17/hdbhatt/ROOTfiles/SIMC_ROOTfiles/";         
  ...
  Double_t E_pi, diff_p, ptd, MissMass2d, pKinW_2, pkinW, z_had;                                //added

  
  //make histograms:
  TH1D *h1_epi_PcointimeROC2    = new TH1D("SHMS ROC2 Corrected epi Coin Time","SHMS ROC2 Corrected epi Coin Time; cointime [ns]",       800,-100,100); 
  TH1D *h1_epi_PcointimeROC2_C  = new TH1D("SHMS ROC2 Corrected epi Coin Time_C","SHMS ROC2 Corrected epi Coin Time_C; cointime [ns]",   800,-100,100);
  TH1D *h1_epi_PcointimeROC2_B  = new TH1D("SHMS ROC2 Corrected epi Coin Time_B","SHMS ROC2 Corrected epi Coin Time_B; cointime [ns]",   800,-100,100);

  // TH1D *h_Q2d = new TH1D("h_Q2d",Form("Q2 (GeV2), Run#:%d", runnumber),100,1,6);  //Q2
  TH1D *h_Q2d = new TH1D("h_Q2d","Q2 (GeV2)",100,1,8);  //Q2
  TH1D *h_Q2s = new TH1D("h_Q2s","Q2 (GeV2)",100,1,8);
  TH1D *h_Q2a = new TH1D("h_Q2a","Q2 (GeV2)",100,1,8); // for accidentals

  TH1D *h_W2d = new TH1D("h_W2d","W2 (GeV2)", 150, 1.0, 15.0 );// W2
  TH1D *h_W2s = new TH1D("h_W2s","W2 (GeV2)", 150, 1.0, 15.0 );
  TH1D *h_W2a = new TH1D("h_W2a","W2 (GeV2)", 150, 1.0, 15.0);

  TH1D *h_xd = new TH1D("h_xd","X_bj", 100, 0., 0.5);   //added                                     
  TH1D *h_xs = new TH1D("h_xs","X_bj", 100, 0., 0.5);    //added                                   
  TH1D *h_xa = new TH1D("h_xa","X_bj", 100, 0., 0.5);    //added                                   

  // if (ptgrp == 1 || ptgrp == 2){
  TH1D *h_zd = new TH1D("h_zd","z_hadr", 100, 0,1.2);   //added                            
  TH1D *h_zs = new TH1D("h_zs","z_hadr", 100, 0,1.2);   //added   
  TH1D *h_za = new TH1D("h_za","z_hadr", 100, 0,1.2);   //added  
  // }
  ...
  // }
  TH1D *h_ptd = new TH1D("h_ptd","pt (GeV)",100,-2,2);   
  TH1D *h_pts = new TH1D("h_pts","pt (GeV)",100,-2,2  );
  TH1D *h_pta = new TH1D("h_pta","pt (GeV)",100,-2,2  );

  TH1D *h_MissMass2d = new TH1D("h_MissMass2d", "MissMass2 (GeV2)", 150, 0,10);
  TH1D *h_MissMass2s = new TH1D("h_MissMass2s", "MissMass2 (GeV2)", 150, 0,10); 
  TH1D *h_MissMass2a = new TH1D("h_MissMass2a", "MissMass2 (GeV2)", 150, 0,10); 
  ...
  epiCut = "P.aero.npeSum > 1.0 && P.cal.eprtracknorm < 0.3 && H.cer.npeSum > 1.0 && H.cal.etottracknorm > 0.5 && H.cal.etottracknorm < 1.8); // && H.cal.eprtracknorm  > 0.2"; // for P>3.GeV, apply P.hgcer....>0.0
  //and ngcer cut as well.
  ...
  for (int kk=0; kk<nentriesD;  kk++){
    ...
    epievent_cut = paeronpe > 1. && pcalepr < 0.3 && pcaletot < 0.5 && hcaletot > 0.6 && hcaletot < 1.8 && hpdelta_cut  && hcernpe > 1.0;
  ...
  // Double_t bincontent_d = h_xd->GetBinContent(5);
  // cout<<"Bin_CONTENT_Data = "<<bincontent_d<<endl;
  // Double_t error_d = h_xd->GetBinError(5);
  // cout<<"Error_Data= "<<error_d<<endl;

  // Double_t bincontent_a = h_xa->GetBinContent(5);
  // cout<<"Bin_CONTENT_ACC_Data = "<<bincontent_a<<endl;
  // Double_t error_a = h_xa->GetBinError(5);
  // cout<<"Error_Acc_Data = "<<error_a<<endl;

  // Double_t bincontent_c = h_xc->GetBinContent(5);
  // cout<<"BIN_CONTENT_cl="<<bincontent_c<<endl;
  // Double_t error_c = h_xc->GetBinError(5);
  // cout<<"ERROR_cl="<<error_c<<endl;

  cout<<"ACC_FAC ="<<acc_scale_fac<<endl;
  ...
  double spdelta, sQ2, sxbj, szhad, wt, sdelta, sfac, sMissMass2, normwt, shxptar,shyptar,spxptar,spyptar, shytar, spytar;
  ...
  h_hdeltac->GetEntries();
  h_hdeltac->GetMean();
  h_hdeltac->GetRMS();
  h_hdeltac->GetIntegral();
  ...
  //  cc2->Print("/w/hallc-scifs17exp/c-sidis18/SIDISAnalyzer/scripts/PDF/spec_comp_LH2_before_Dummysub_3_12.pdf");
  //***************
  // if (numruns > 1 ){
  ...
  // cc->Print("/w/hallc-scifs17exp/c-sidis18/SIDISAnalyzer/scripts/PDF/kine_comp_LH2_before_Dummysub_3_12.pdf");
