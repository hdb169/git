...
Int_t runs[NRUNS]=        { 6081,6068,6070,6071,6073,6081,6080,6082,6083,6088,6089,6090,6087,6084,6085,6086,6545,6546,6546,6548,6549,6547,6111,6112,6114,6109,6110,6104,6105,6106,6486,
...
#include "scaler_analysis.h"
    ...
    for (int kk=0; kk<nentriesD;  kk++){//-----------------------------------------------------Main Evt Loop
      ...
      hRF_clean_neg->Add(hRF_coin,hRF_acc_1peak,1,-1);
      ...
      hRF_coin->Draw("hist");
      hRF_clean_neg->Draw("hist sames");
      hRF_acc->Draw("hist sames");
      hRF_coin->SetLineWidth(2);
      hRF_acc->SetLineWidth(2);
      hRF_clean_neg->SetLineWidth(2);
      hRF_coin->SetLineColor(kBlack);
      hRF_acc->SetLineColor(kRed);
      hRF_clean_neg->SetLineColor(kBlue);
      hRF_coin->GetXaxis()->SetAxisColor(17);
      hRF_coin->GetYaxis()->SetAxisColor(17);
      hRF_coin->SetTitle(Form("RF Time %d",runs[r]));
      RF->SaveAs(Form("pdf_rf/rf_hists_neg_%d.pdf",runs[r]));
    
  }
}
//positive Int_t runs[NRUNS]=        {6020,6021,6022,6023,6026,6024,6025,6026,6029,6030,6031,6032,6027,6028,6033,6034,6035,6036,6043,6044,6045,6046,6039,6038,6040,6041,6042,6537,6538,6040,6041,6042,6537,6538,6541,6539,6145,6146,6154,6155,6168,6142,6144,6163,6164,6500,6501,6502,6194,6195,6196,6192,6193,6198,6199,6197,6200,6201,6419,6420,6421,6422,6423,6525,6426,6429,6430,6431,6432,6433,6427,6428,6309,6310,6311,6312,6313,6314,6387,6388,6397,6398,6386,6389,6518,6519,6520,6521,6535,6536,6533,6534,7635,7636,7637,7638,7639,7699,7702,7703,7706,7707,7708,7709,7710,7711,7712,1163,7760,7762,7765,7766,7768,7769,7770,7771,7772,7808,7809,7811,7812};
