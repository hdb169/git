//This macro produces the yield, livetime, tracking, cal eff, etc with perform acc subtraction.
//root -l plot_rftime_eff.C
//Enter the mom of the run to get the time of the pion, kaon, and proton
...
void plot_rftime_eff(){
  ...
  Int_t runs[NRUNS]={7808,  7796 };//+d2,+dum -d2 -dum//all good
...
#include "scaler_analysis.h"
	...
	if(acc_cut_norf && hgcer > 2){//accidental in RF
	  rf_1da->Fill(rf_pion);
	}
	if(hms_cut1 && shms_cut1 && pcal_cut1 && ctime_cut1 && hgcer > 2){//accidental in RF
	  rf_1d->Fill(rf_pion);
	}
	///delete this
